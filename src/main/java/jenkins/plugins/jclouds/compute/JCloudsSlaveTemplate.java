...
public class JCloudsSlaveTemplate
    implements Describable<JCloudsSlaveTemplate>, Supplier<NodeMetadata> {
  ...
  public JCloudsSlaveTemplate(final String name, final String imageId, final String imageNameRegex,
                              final String hardwareId, final double cores, final int ram,
                              final String osFamily,
                              final String osVersion, final String labelString,
                              final String description, final String initScript,
                              final String userData,
                              final String numExecutors, final boolean stopOnTerminate,
                              final String vmPassword, final String vmUser,
                              final boolean preInstalledJava,
                              final String jenkinsUser, final boolean preExistingJenkinsUser,
                              final String fsRoot, final boolean allowSudo,
                              final int overrideRetentionTime,
                              final int spoolDelayMs, final boolean assignFloatingIp,
  ...
  public JCloudsSlave provisionSlave(TaskListener listener) throws IOException {
    ...
    try {
      return new JCloudsSlave(getCloud().getDisplayName(), getFsRoot(), nodeMetadata, labelString,
                              description, numExecutors, stopOnTerminate,
  ...
  public NodeMetadata get() {
    ...
    if (!Strings.isNullOrEmpty(vmPassword)) {
      ...
      LoginCredentials
          lc =
          LoginCredentials.builder().user(vmUser).privateKey(getCloud().privateKey).build();
      options.overrideLoginCredentials(lc);
    }
    ...
    if (spoolDelayMs > 0) {
      ...
      synchronized (delayLockObject) {
        LOGGER.info("Delaying " + spoolDelayMs + " milliseconds. Current ms -> " + System
            .currentTimeMillis());
        try {
    ...
    if (this.preExistingJenkinsUser) {
      ...
      AdminAccess
          adminAccess =
          AdminAccess.builder().adminUsername(getJenkinsUser()).installAdminPrivateKey(
              false) // no need
              .grantSudoToAdminUser(allowSudo) // no need
              .adminPrivateKey(getCloud().privateKey) // temporary due to jclouds bug
              .authorizeAdminPublicKey(true).adminPublicKey(getCloud().publicKey).adminHome(
              getFsRoot()).build();

      // Jenkins needs /jenkins dir.
      Statement
          jenkinsDirStatement =
          Statements.newStatementList(Statements.exec("mkdir -p " + getFsRoot()),
                                      Statements.exec(
                                          "chown " + getJenkinsUser() + " " + getFsRoot()));

      initStatement =
          newStatementList(adminAccess, jenkinsDirStatement, Statements.exec(this.initScript));
    }
    ...
    if (userData != null) {
      try {
        ...
        LOGGER.log(Level.WARNING,
                   "userData is not supported by provider options class " + options.getClass()
                       .getName(), e);
      }
    }
  ...
  private RuntimeException destroyBadNodesAndPropagate(RunNodesException e) {
    for (Map.Entry<? extends NodeMetadata, ? extends Throwable> nodeError : e.getNodeErrors()
        .entrySet()) {
  ...
  public static final class DescriptorImpl extends Descriptor<JCloudsSlaveTemplate> {
    ...
    public FormValidation doValidateImageId(
      ...
      try {
        ...
        Set<? extends Image>
            images =
            listImages(providerName, identity, credential, endPointUrl);
        if (images != null) {
    ...
    public FormValidation doValidateImageNameRegex(
      ...
      return FormValidation.error(
          "Invalid Image Name Regex, please check the value and try again.");
    }
    ...
    private Set<? extends Image> listImages(
      ...
      try {
        computeService =
            JCloudsCloud.ctx(providerName, identity, credential, endPointUrl).getComputeService();
    ...
    public ListBoxModel doFillHardwareIdItems(
      ...
      try {
        ...
        Set<? extends Hardware>
            hardwareProfiles =
            ImmutableSortedSet.copyOf(computeService.listHardwareProfiles());
    ...
    public FormValidation doValidateHardwareId(@QueryParameter String providerName,
      ...
      FormValidation
          result =
          FormValidation.error("Invalid Hardware Id, please check the value and try again.");
      ComputeService computeService = null;
      try {
        // TODO: endpoint is ignored
        computeService =
          ...
          if (!hardware.getId().equals(hardwareId)) {
            if (hardware.getId().contains(hardwareId)) {
              return FormValidation.warning(
                  "Sorry cannot find the hardware id, " + "Did you mean: " + hardware.getId()
                  + "?\n" + hardware);
            }
          } else {
        ...
        result =
            FormValidation.error("Unable to check the hardware id, "
